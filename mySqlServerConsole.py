    ...
    def showDatainFrame(strQuery,newFrame):
        try:
            ...
            for rowHeader in myCursor.description:
                Label(newFrame, text=rowHeader[0], relief=RAISED).grid(row=r, column=c, sticky=W + E + N + S)
                c += 1
            r += 1
...
#Method of Main Form
    def createWidget(self):
        ...
        self.lblHostName.grid(row=1,column=0,sticky=W,columnspan=3)
        self.varHostName=StringVar()
        self.varHostName.set("localhost")
        self.txtHostName=Entry(self,textvariable=self.varHostName)
        self.txtHostName.grid(row=1,column=3,sticky=W,columnspan=3)

        self.lblUser = Label(self, text="User:")
        self.lblUser.grid(row=2, column=0,sticky=W,columnspan=3)
        self.varUser = StringVar()
        self.varUser.set("pathak")
        self.txtUser = Entry(self, textvariable=self.varUser)
        self.txtUser.grid(row=2, column=3,sticky=W,columnspan=3)

        self.lblPassword = Label(self, text="Password:")
        self.lblPassword.grid(row=3, column=0,sticky=W,columnspan=3)
        self.varPassword = StringVar()
        self.varPassword.set("arpitdadlove")
        self.txtPassword = Entry(self, textvariable=self.varPassword,show='*')
        self.txtPassword.grid(row=3, column=3,sticky=W,columnspan=3)

        self.btnConnect=Button(self,text="Connect",command=self.btnConnect_Click)
        self.btnConnect.grid(row=4,column=2,columnspan=3)
